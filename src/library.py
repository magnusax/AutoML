...
def expose_library(names, nrow, ncol):
    library = {'logreg': _logreg_lib(nrow, ncol),
    ...
    return [(name, grid) for name, grid in library.items() if name in names and grid is not None]
        

# Linear models
def _logreg_lib(nrow, ncol):
    grid = {'category':'continuous', 'method':'sample', 
...
def _randforest_lib(nrow, ncol):
        values = [v for v in (1, 2, 4, 6, 8, 12, 16, 20, 24) if v <= ncol]
    return [{
        'param': 'max_features',
        'grid': {'category': 'discrete', 'metod': 'take', 'values': values},
        'premise': {'n_estimators': 1024},
    }]
